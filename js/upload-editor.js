...
function openBatchModal(mode) {
  ...
  if (typeof renderOcrDebugPanel === 'function') renderOcrDebugPanel(null);
  ...
  for (const rec of targets) {
    ...
    editorQueue.push({
      ...
      data: {
        ...
        totalMiss: rec.missCount, totalNotes: (rec.perfect || 0) + (rec.great || 0) + (rec.good || 0) + (rec.bad || 0) + (rec.miss || 0),
        combo: rec.combo,
        musicId: rec.musicId,
        deviceProfileId: profile ? profile.id : null,
      },
...
function selectItem(id) {
  ...
  if (typeof renderOcrDebugPanel === 'function') renderOcrDebugPanel(item);
}
...
function updateCurrentItem(field, value) {
  ...
  if (['perfect', 'great', 'good', 'bad', 'missDetail'].includes(field)) {
    item.data.totalNotes = (item.data.perfect || 0) + (item.data.great || 0) + (item.data.good || 0) + (item.data.bad || 0) + (item.data.missDetail || 0);
    if (field === 'good' || field === 'bad' || field === 'missDetail') {
      ...
      if (totalMissEl) totalMissEl.innerText = item.data.totalMiss;
    }
  }
...
async function runBatchAnalysis(itemsToAnalyze) {
  ...
  for (const item of itemsToAnalyze) {
    ...
    try {
      ...
      const res = await analyzeLoadedImage(img, worker, regions, { profileId: profile ? profile.id : null, profileName: profile ? profile.name : '' });
      if (res) {
    ...
    if (item.status === 'done') {
      ...
      item.debugLog = item.debugLog || null;
      if (activeItemId === item.id && typeof renderOcrDebugPanel === 'function') renderOcrDebugPanel(item);
    }
  }
