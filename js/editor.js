...
const EditorScreen = (() => {
  ...
  function _tagChip(tag, onRemove) {
    ...
    colorBtn.title = 'タグ色を変更';
    colorBtn.style.background = tag.color || '#3b82f6';
    ...
    input.type = 'color';
    input.value = tag.color || '#3b82f6';
    input.className = 'chip-color-input';
    input.addEventListener('input', async e => {
      const color = e.target.value;
      tag.color = color;
      AppState.updateTag({ id: tag.id, color });
      await Storage.saveTag(AppState.getTag(tag.id));
