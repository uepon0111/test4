...
const PlayerScreen = (() => {
  ...
  async function _renderStageItem(entry) {
    ...
    try {
      ...
      entry.peakLevel  = meta.peakLevel || 0;
  ...
  async function _uploadFiles() {
    ...
    for (const entry of pendingFiles) {
      ...
      try {
        ...
        const normalizationGain = entry.peakLevel > 0 ? clamp(0.85 / entry.peakLevel, 0.5, 4.0) : 1;
        const track = {
          id: entry.id, title: entry.title, artistIds, tagIds: [],
          releaseDate: entry.releaseDate, thumbnail: entry.thumbnail,
          duration: entry.duration, peakLevel: entry.peakLevel || 0,
          normalizationGain, addedAt: Date.now(), manualOrder: AppState.tracks.length,
        };
