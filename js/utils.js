...
// カタカナ(全角)をひらがなに変換する。楽曲名の読み方(pronunciation, ひらがな)と、
// 日本語IMEが変換確定前に生成しうるカタカナ入力の両方を同じ形に揃えて比較するために使う。
function katakanaToHiragana(str) {
  return (str || '').replace(/[\u30a1-\u30f6]/g, ch => String.fromCharCode(ch.charCodeAt(0) - 0x60));
}

// 曲名検索(タイトル登録・ページ内検索)用の正規化。normalizeString に加えてカタカナ→ひらがな
// 変換を行うことで、「ろき」「ロキ」のどちらの入力でも同じキーとして比較できるようにする。
// (ローマ字→ひらがなの変換自体はブラウザ・IME側が担うため、ここでは行わない)
function normalizeForKanaSearch(str) {
