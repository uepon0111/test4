...
// 曲名検索の一致判定 (ひらがな読み対応)
// ------------------------------------------------------------
// レコード自身のタイトル文字列に加えて、musicId が紐付いていればマスターDBの
// ひらがな読み(pronunciation)も対象にする。「ろき」と入力しても「ロキ」がヒットする。
// ============================================================
function recordMatchesTitleQuery(rec, normalizedQuery) {
  if (!normalizedQuery) return true;
  if (normalizeForKanaSearch(rec.title).includes(normalizedQuery)) return true;
  if (rec.musicId) {
    const m = getMusicById(rec.musicId);
    if (m && m.pronunciation && normalizeForKanaSearch(m.pronunciation).includes(normalizedQuery)) return true;
  }
  return false;
}
...
async function updateView() {
  ...
  const tiF = normalizeForKanaSearch(document.getElementById('filter-title').value);
  ...
  list = list.filter(r => {
    ...
    if (!recordMatchesTitleQuery(r, tiF)) return false;
    return true;
  });
...
// 絞り込み・並び替えをやり直す必要は無いが、カードのHTML(選択モード用のオーバーレイや
// 選択状態のハイライト等)を再描画したい場合に使う軽量な再描画 (selection-mode.js から使用)。
async function renderGrid(records) {
  const myToken = ++renderToken;
  await renderGridChunked(records || filteredRecords, myToken);
  if (myToken === renderToken) hideLoadProgress();
}
...
function initFilterControls() {
  ...
  // 曲名検索: マスターDBからのリアルタイム候補(ひらがな読み対応)。
  // 候補は「自分が実際に記録を持っている曲名」だけに絞り込む(全曲DBを出しても
  // 手持ちのデータに無ければ検索結果が0件になるだけで実用的でないため)。
  attachAutocomplete(document.getElementById('filter-title'), {
    getSuggestions: (q) => searchMusicCandidates(q, 8, { onlyTitles: getOwnedTitleSet() }),
...
// 現在保持しているレコードの曲名一覧(正規化済み)。曲名検索候補の絞り込みに使う。
function getOwnedTitleSet() {
  const set = new Set();
  (allRecords || []).forEach(r => { if (r.title) set.add(normalizeForKanaSearch(r.title)); });
  return set;
}
