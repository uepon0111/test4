...
function setupBackLinkBehavior() {
  const backLink = document.getElementById('settings-back-link');
  if (!backLink) return;
  if (window.parent && window.parent !== window && typeof window.parent.closeSettingsModal === 'function') {
    backLink.addEventListener('click', (e) => {
      e.preventDefault();
      window.parent.closeSettingsModal();
    });
    backLink.innerHTML = '<span class="material-symbols-outlined">close</span> 閉じる';
  }
}
