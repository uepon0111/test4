...
function getMusicDifficultyRecord(musicId, diffKey) {
  if (!musicId || !diffKey || !dbDiffs) return null;
  return dbDiffs.find(d => d.musicId === musicId && d.musicDifficulty === diffKey) || null;
}

function getNotesTotalFromDb(musicId, diffKey) {
  const entry = getMusicDifficultyRecord(musicId, diffKey);
  if (!entry) return null;
  const candidateKeys = ['totalNotes', 'total_note', 'total_note_count', 'totalNoteCount', 'noteCount', 'notesCount', 'note_count', 'notes'];
