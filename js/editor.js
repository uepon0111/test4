...
const EditorScreen = (() => {
  ...
  function _chip(label, type, onRemove, color, tagId) {
    ...
    if (type === 'tag' && tagId) {
      ...
      /* input: ドラッグ中はチップの背景色だけをリアルタイム更新（保存はしない） */
      swatch.addEventListener('input', e => {
        c.style.background = e.target.value;
      });

      /* change: ドラッグ終了時に1回だけ保存・更新・トースト表示する。
         一部ブラウザ・OSでは change が連続発火するため debounce で抑制する。 */
      const _debouncedSave = debounce(async (newColor) => {
      ...
      swatch.addEventListener('change', e => _debouncedSave(e.target.value));
